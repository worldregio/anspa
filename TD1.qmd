...
On considère un monde composé de 48 positions spatiales, chacune d'entre elles étant occupée par une carte à jouer d'un jeu de 52 cartes dont on a retiré les rois. Cinq tirages différents ont été effectués au hasard.
...
Nous avons quatre familles d'individus correspondants à Pique, Coeur, Carreau et Trèfle. Proposez une méthode pour répondre à la question 

> **Laquelle de ces familles est la plus concentrée spatialement ? La plus dispersée ?**

## Solution 1 : Point moyen,  distance-type et ellipse de dispersion

Vous avez sans doute appris en licence quelques bases de **statistique spatiale**. Celle-ci propose notamment de calculer pour chaque distribution de points de coordonnées $(X_i,Y_i)$ le centre de gravité $G(X_g,Y_g)$ :
...
library(kableExtra)
kable(tabres, digits=2) %>% row_spec(c(3,4), color = "red") 
```

On déduit facilement de ce tableau la distribution la plus dispersée et la moins dispersée.
...
Examinons la distribution des individus de la famille "Pique"`{r} rownames(tabres)[1]` :
...
::: {.callout-important title="Commentaire :"}
Cette distribution est *globalement plus dispersée* que la précédente puisque la distance moyenne entre deux individus est de 5.86 contre 4.32. Mais elle est *localement plus concentrée* puisque chaque individu a systématiquement deux voisins
:::


## Solution 3 : Graphe et composantes connexes

Une troisième solution (qui devrait rappeler les règles du jeu de go à ceux et celles qui le connaissent) consiste à déterminer le degré de fragmentation de chaque famille en **composantes connexes**.  On considère désormais que deux individus d'une même famille sont reliés s'il existe un chemin les reliant qui ne passe que par des membres de la même famille. On va donc définir un **graphe de voisinage** entre les cellules et considérer que la relation est possible si des membres de la même famille se trouvent de part et d'autre.


Ceci revient à déterminer toutes les paires contiguës $(C_{ij}=1)$ puis à vérifier si elles relient des individus similaires $(S_{ij}=1)$ pour déterminer si la relation entre eux est possible $(R_{ij}=1)$. On trouve 44 paires orientées soit 22 paires symétriques vérifiant les conditions.
...
# EXO2 : DYNAMIQUES DE SÉGRÉGATION ET D'OPINION


## Mise en place

Une quarantaine d'étudiant·es entrent dans une salle de cours comportant 6 rangées de 8 chaises. Sans dire un mot, les enseignant·es leur remettent silencieusement une carte à jouer tirée d'un jeu de 52 cartes d'où les rois ont été retirés et leur demandent de s'installer. Les enseignant·es procèdent à l'appel en suivant l'ordre des rangées et chaque étudiant·e indique la carte à jour qui lui a été attribuée, tandis que l'enseignant·e lui indique sa position spatiale :

- **Étudiant·e** : Je suis Camille Dupont et ma carte est le 8 de Pique.
- **Enseignant·e** : Vous occupez la place C36.


Après avoir saisi la position de tous les étudiant·es, les enseignants effectuent quelques manipulations sur leur ordinateur et affichent la carte suivante à l'écran. On notera qu'il existe un certain nombre de places vides qui vont jouer un rôle important dans les modèles de ségrégation et de diffusion d'opinion. 
...
On voit donc que dans ce modèle, les individus ont une opinion qui varie en fonction de leur opinion initiale et de celle de leur(s) voisin(s). Ils ont d'autant plus de chances de garder leur opinion qu'ils sont entourés d'individus ayant la même opinion. Et inversement d'autant moins de chance qu'ils sont entourés d'individus ayant des opinions différentes. 

### Simulation de la dynamique

> L'exercice est à faire en classe avec les étudiant·es. Mais on peut donner ici un exemple de résultat.
...
4. Si la proportion de voisins différents est supérieure à un seuil $\alpha$, l'individu se déplace vers une cellule vide où le seuil est respecté. 


Supposons que le seuil $\alpha$ soit égal à 0.70, c'est à dire qu'un individu accepte que 70% de ses voisins soient différents de lui. Combien y-a-t-il d'individus mécontents du voisinage dans la configuration suivante en supposant que l'on considère comme voisin uniquement ceux qui ont une frontière commune : 
...
Chaque étudiant·e évalue sa satisfaction par rapport à son voisinage et décide en fonction de cela de bouger ou non. Voici quelques exemples de calcul de la satisfaction et de la décision de bouger ou non. 

- Le <span style=color:red>7♥</span> est rouge et possède un voisin rouge donc 0% de ses voisins sont différents : il ne bouge pas.
- le <span style=color:red>4♥</span> possède deux voisins rouges et un voisin noir donc 33% de ses voisins sont différents : il ne bouge pas.
- Le 4♠ possède un voisin rouge et un voisin noir donc 50% de ses voisins sont différents : il ne bouge pas.
- la <span style=color:red>D♦</span> possède trois voisins noirs et un voisin rouge donc 75% de ses voisins sont différent : elle va bouger.
- le 2♠ possède trois voisins rouges et aucun voisin noir donc 100% de ses voisins sont différents : il va bouger.


Le problème est que chaque déplacement d'un individu modifie la table de voisinage et remet en cause les calculs. On ne peut donc pas effectuer des changements simultanés comme dans le cas du modèle d'Axelrod. Il faut ici utiliser un modèle asynchrone où les individus insatisfaits se déplacent un par un et où chaque individu recalcule sa situation avant de se déplacer à son tour.


Supposons par exemple que la <span style=color:red>D♦</span> effectue un déplacement de C43 en C20 pour rejoindre la case située entre la <span style=color:red>D♥</span> et le <span style=color:red>9♦</span> :
...
- **au point de départ** 
  + Le <span style=color:red>3♦</span> qui était satisfait (33% de voisins similaires) a désormais uniquement des voisins noirs et va décider de partir
  + le 6♠ qui était mécontent (100% de voisins différents) ne l'est plus et décide de rester
  
- **au point d'arrivée**
  + La <span style=color:red>D♥</span>qui était mécontente (100% de voisins différents) décide de rester
  + la D♠ décide de partir...
  + etc.
...
De nombreux programmes de simulation existent en ligne pour simuler l'évolution des modèles de Schelling. Voir par exemple [ce site](http://nifty.stanford.edu/2014/mccown-schelling-model-segregation/){target=_blank}, du colloque de l'*Association for Computing Machinery's Special Interest Group on Computer Science Education*.
...
- **Problème 1** : Essayez de répartir au mieux les déplacements des individus vers le service le plus proche tout en respectant les capacités maximales d'accueil des commerces ou des services. Quelle distribution spatiale vous semble la plus intéressante entre "<span style=color:red>♥</span>" et "♠" ? Justifiez votre réponse. 
...
- **Problème 3** : En supposant qu'il s'agit de **commerces ou services privés** (e.g. pharmacies) soumis aux lois de la concurrence, quelles décisions pourraient prendre les acteurs pour augmenter leur part de marché ? Dans quelle mesure ces décisions seraient-elles également profitables aux client·es ?
