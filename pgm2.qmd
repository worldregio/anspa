...
# Calcul de l'accessibilité moyenne pondéree
moy = sum(tab$dis*tab$pct)/100
paste("L'accessibilité moyenne est de ", round(moy,1), "minutes")
...
text(moy,100,paste("moyenne = ",round(moy,1), "mn"),col="blue")
grid()
```
...
g<- ggplot(tab) + aes(x=dis, y=pctcum) +
                  ...
                  geom_vline(xintercept=moy,col="blue")+
                  geom_text(aes(x=moy,y=100, label=paste("moyenne = ",round(moy,1), "mn")))+
...
On peut l'effectuer sur les supports de son choix (Qgis, Magrit, etc, ...). Le résultat devrait ressembler à ceci :
...
map2 <- mapview(grille, 
                zcol = "duree",
                cat = c(0, moy, 2*moy, 3*moy, max(moy)),
                col.regions=c("green","lightyellow","yellow","orange"),
                legend = T)
map<-map2+map1
map
```
